...
. Copy `bd.data` from the "`Etcetera Disk`" to the "`Program Disk`" and rename it to "`d.data`".
. Copy `e.lightdefs`, `e.lightbar` and `e.ecs.main` from IMAGE 3.0 D2 to your "`Etcetera Disk`" replacing the existing files of the same name.
...
. When prompted, type kbd:[3] to `Convert a 2.0 system`.

. Image 3.0 will perform all the necessary tasks of converting your 2.0 to 3.0 and will end up at the idle screen.
Since the 3.0 modem configuration is different from 2.0, Image will not be communicating with your virtual modem.

. Log on to your BBS from the console and enter kbd:[IM] to go to the Configuration Editor.

. Configure your modem.

include::sub_modem-config.adoc[]
...
TIP: This takes you to the menu page that used to be called by the kbd:[SM] command in 2.0.
...
You will need to reconfigure your GF section due to the incompatibility between 3.0 and 2.0 program files.
