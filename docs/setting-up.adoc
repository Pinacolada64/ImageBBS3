...
type kbd:[1] to `Configure a New BBS`.
...
Image may (if the date entered was different than the old date) or may not perform AutoMaintenance again and return to the System Idle screen.

### Console Login

A _console login_ is when you're sitting in front of the C64.
...
For more information, see the relevant `IM` documentation.
****
...
Logging off by hitting kbd:[f7] (unchecking left `Loc`) bypasses the module which updates the last 10 callers and cause errors in the time used during the call.
...
The BBS loads the access group information.
...
.. At minimum, you need to set the `Time Zone` and the `System ID`.
..  If you are running on a Lt. Kernal, type kbd:[A] to set your Lt. Kernal device number.
.. Type kbd:[M] to configure your Chat messages for entering and leaving chat.
...
If your 1.2 system was networked, go into each netsub you have defined.
...
9.	If you had macros defined in your 1.2 system and don't want to keep them, delete the `e.macros` file.

If you would like to convert the file, follow these steps:

#FIXME# more exact steps.

.. Type kbd:[RD] at the Main Prompt, and read the SEQuential file `e.i.macros`.
...
Load and run the `run me` file (Image SEQ Reader) which allows you to read the installation instructions for all the games included on the disk.
