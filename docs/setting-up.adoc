...
It will support single or dual drives, or logical units (LUs) 0 through 9 on a Lt. Kernal (IMAGE will not access LU 10 on a Lt. Kernal).
...
- Combine 3 (Etcetera), 4 (Directory) and 6 (User) drives to a different drive or partition.
...
[start=1]
.	#Press kbd:[f1] to go to full screen mode then# 
. Use kbd:[f3] (move highlight left) and kbd:[f5] (move highlight right) to move the lightbar highlight to `Loc`.
. Press kbd:[f7] to put a check mark on the left of `Loc`.

This starts a console BBS login.

### Instant Login

The BBS copyright message is displayed, and 


. Press kbd:[I] for Instant Login. 

. When asked, type your sysop password.
