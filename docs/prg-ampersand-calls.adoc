...
A new `&` command, `&,15,2`, does the same thing when passed the string to translate in `an$`.
...
====
TIP: See <<position-cursor>> for how to easily do that within Image BBS.
====
...
* `&pound;m<__x__` (left margin) This MCI command causes every carriage return issued by Image BBS to be followed by _x_ spaces, which indents text _x_ spaces.
The values for _x_ are `0` (to disable word-wrap), or `1-9`, and `j-o` (10-15).

* `&pound;m>__x__` (right margin).
...
The use of `&pound;m<__x__` and `&pound;m>__x__` together allows you to make "`block indents`" of text that appear correct regardless of the user's screen width.
...
This worked under Image 1.2; it works here too, but there is a new MCI command in Image 3.0 to output single-letter floating point variables:
...
However, the parameter _[,optional]_ is optional, and if supplied, needs `,` and to substitute the appropriate parameter in its place.
...
<1> `poke 53252,20`: Set the input length to 20 characters. `w$=na$`: Assign the user's handle (`na$`) to `w$` (the prompt's default). `p$="Handle"`: Set the prompt to `Handle:`. `&,1,32`: Setting bit 5 (a value of 32) enables edit mode. The string typed in response to a prompt is returned in `an$`. `if an$<>"" then na$=an$`: Just typing kbd:[Return] (to accept the default, `w$`) sets `an$` to a null string. If something else was typed (`an$<>""`), assign `na$` to what was typed (`an$`).
...
<4> `&,2,2,**25**`: using logical file #2, get a line of data from the disk--but stop at *25* characters, ignoring a carriage return.
The data is returned in `a$`.
`&`: output `a$`.
...
Sets input length to 14, and uses an input mask.
The mask character is displayed, rather than the real character typed.

_Parameters:_

`poke 17138,_mask_`: display this character instead of the user's input
...
<2> `&,8,**2**,0`: Use lfn#2 (`2`) to get entire disk directory. `close 2`: close lfn#2. `goto 300`: go to main prompt.
...
[%header]
|===
| Position | Purpose | Range 
| 1     | Weekday | `1`=Sun...`7`=Sat 
| 2-3   | Month   | `01`...`12`
| 4-5   | Date    | `01`...`31`
| 6-7   | Year    | `00`...`99` (plus internal century)
| 8-9   | Hour    | `01`...`12` (add `80` for PM)
| 10-11 | Minute  | `01`...`59`
|===
...
* `a$="20517218944":&"kbd:[Ctrl+d]"+a$`
* `a$="20517218944":&chr$(4)+a$`
...
This splits `"Hello world"` at `**chr$(32)**` (kbd:[Space]), resulting in `an$="Hello"` and `a$="world"`.

[start=2]
. `an$="Hello world":&,15,6,**asc(" ")**`

This splits `"Hello world"` at the **ASCII value of kbd:[Space]** (32), resulting in `an$="Hello"` and `a$="world"`.

After the split, the two strings look like this:

[%autowidth]
|===
| `an$="Hello"` | `chr$(32)` | `a$="world"` 
|===
...
This splits `"Hello world"` at the `" "` (space), resulting in `an$="Hello"` and `a$="world"`.
////
****
...
====
Jump tables are useful for giving assembly language routines entry points that don't change.
In other words, if a particular routine changes in size, that would shift all the addresses to jump to around from that point forward.

You probably don't want to search for all the BASIC `sys` references to the changed addresses throughout your code.
Instead, just change the address that the jump table entry points to, and you can keep the BASIC `sys` address that calls the routine the same (stable).
====
...
TIP: Refer to xref:prg-machine-language.adoc#protocols[Protocols] for more information.
...
This is meant to possibly eventually replace ``peek()``ing memory locations based on frequency of use.
...
_Returns:_ `$61`: Start of buffer holding the variable contents.
...
BASIC run-time errors will be caught, and redirected to the BBS error handler at line 
---
...
TIP: `&,48` does not stop and wait for input, unlike the `&pound;g1` MCI command.
...
* clears status indicators on status line
...
Structures (or _structs_) are 

See the xref:prg-structures.adoc#structures[Static Arrays (Structures)] chapter for more information.
...
If you have 14 4-digit `goto` targets on a line, this can save a fair amount of RAM (and BASIC interpretation time).
...
If you have 14 4-digit `gosub` targets on a line, this can save a fair amount of RAM (and BASIC interpretation time).
