...
<1> `dr=1:gosub 3` get device of s. disk{newline}bla
...
`i.t`::
`an$="hostkbd:[f3]port":&,15,2:&"{pound}v7{f6}"` -> host:port
...
This group of commands is used for searching, loading, saving and the Extended Command Set (ECS).
...
`&,42,3` Put ECS definitions into RAM
...
`&,42,4` Load ECS definitions from disk
 ...
 a$=dr$+"e.ecs.main":&,42,4
 
im 3106: loads `e.ecs.main`
...
`option`=3: read status of _position_, return in `a%`
...
Cause BASIC error #_error_ to happen.
...
`&,53,1,14` Cause error `14`, `?illegal quantity&nbsp;&nbsp;error`.
...
Throw error _a_, which crashes BBS.
