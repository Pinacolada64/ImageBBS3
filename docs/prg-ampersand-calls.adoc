...
In BASIC, the `print` keyword (or its abbreviation, `?`) displays text on the screen.
For example:
...
==== Encoded Function Keys [[encoded-function-keys]]
...
Bits may be combined to perform multiple functions.

.Input Line: Editor Control Bits
[%header]
[%autowidth]
[cols=">1,>2,<3"]
|=====
...
| Variable | Purpose
| `an$`
| a copy of the text the user typed.
Is a null string (`""`) if the user did not type anything and just hit kbd:[Return] or kbd:[Enter].
|=====
...
|=====
| `peek()` | Purpose
| `$d006` / `53254` | 0: normal exit

1: kbd:[Delete], kbd:[.] or kbd:[/] typed on column 1

2: Left `Cht` check on lightbar was activated by sysop

3: Loss of carrier, or time expired (`tr%<1`)
...
.Input Line: Password Entry Control Bits
[%header]
[%autowidth]
|=====
...
.Input Line: Set Line Length and Prompt
[source,basic]
...
.Input Line: Set Line Length, Default Text, Prompt and Edit Mode
[source,basic]
...
.Input Line: Set Line Length, Default Text, Prompt and Allowing MCI & Graphics Characters
[source,basic]
...
<2> a 30-character string (with `+` every 5 characters to simplify keeping track of string position), used to demonstrate `&,2,2,25`
...
`goto 3100`: close the disk channel instead of getting data from a non-existent file.
...
// is it possible to continue getting data from there with &,2,2 again?

<5> `close 2`: close disk file.
`goto 300`: go to main prompt.

.Details: BASIC Disk Data Pitfalls
****
Using `input#2,a$` when the disk file contains a string `hello,there` returns only `hello` in `a$`.
When encountered by `input#`, `,` is a delimiter which truncates (cuts off) the data after the `,`.
...
=== `&,5` Get Version [[amp-5-get-version]]
...
.Details: Segments
****
Segments are pre-defined addresses that a module will load to, regardless of the file's load address (the first two bytes of the file).
...
Check for a DOS error and `return` from the subroutine.
****
...
* a line of information at a time.

Calling it multiple times will get the directory header, then each directory entry (the file`'s block count, filename, filetype, splat and lock status), and finally the `blocks free.` count)
...
`st`: `if 64 and st then ...` (checking if bit 7 is `1`) checks for the end of the directory.

====
TIP: The reason the `if` statement is not written `if st and 64` is because of how the BASIC interpreter tokenizes keywords.
If there were no spaces between keywords here, `ifstand64` gets tokenized by the BASIC interpreter as `if s tan d 64` (`tan` being a mathematical function).
...
`open 2,dv%,**0**,"$"+dr$+"\*"`: open the directory as a file.
The secondary address must be **`0`** to instruct the drive to return the disk directory as a BASIC-formatted series of lines, displayable by this routine.
`$0:*`: Use the wildcard pattern `*` (all files).
`get#2,a$,a$`: discard the directory's load address information.
...
====
[CAUTION]
There is currently a icon:bug[] with displaying integer or floating-point variables to the programmable window.
It seems only the memory adjacent to the requested variable's descriptor is displayed.
This results in displaying 16 bytes of more-or-less random binary data; the variable's address that the descriptor points to needs to be resolved.
A fix is being looked into.
====
...
Write `tt$()` array elements to an already-open file, using the specified logical file number.
...
====
NOTE: See <<table-array-numbers>> for the arrays which correspond to _array_.
====

'''

=== `&,15` Convert `an$` [[convan]]

This group of functions perform various conversions on the string contained in `an$`.

TIP: Related string conversion functions can be found at  <<convert>>.
...
Changes an all-uppercase string (such as a user's handle) to mixed case.
Characters after a space are changed to uppercase, all other characters are changed to lowercase.
...
Based on code from `i.t` (the Image terminal)::
`an$="hostkbd:[f3]port":&,15,2:&"£v7kbd:[f6]"`
...
.Details: Split on Space, Two Ways
****
[start=1]
. `an$="Hello world":&,15,6,**32**`

This splits `"Hello world"` at `**chr$(32)**` (kbd:[Space]).

[start=2]
. `an$="Hello world":&,15,6,**asc(" ")**`

This splits `"Hello world"` at the **ASCII value of kbd:[Space]** (32).

After the split, both examples produce two strings which look like this:
...
This line parses the string `an$` which holds a sequence of individual stacked commands.
Each command in the string is separated by an kbd:[f8] character, which is `^` in Image BBS`'s special character encoding format.

.Command Stacking Example
 SB4,3^R1^>^RN

[start=1]
. `&,15,6,140`: first, check `an$` for `chr$(140)` (kbd:[f8], `^`).
Assign from the start of the string to before the kbd:[f8] to `a$`.
Assign from after the first occurence of kbd:[f8] to the end of the string to `an$`.

In our example, it would be `a$="SB4,3"`, and `an$="R1\^>^RN"`.

[start=2]
. `uu$=an$`: next, assign `an$` (the remainder of the stacked commands in the sequence) to `uu$` (used for command stacking elsewhere)

. `an$=a$`: lastly, since the remainder of the stacked command string after the `^` is now stored in `a$`, `a$` is assigned back to `an$` in order to continue parsing the stacked commands.

In our example, it would be `an$="R1\^>^RN"`.

This process repeats until the entire string is parsed.
...
====
INFO: Refer to xref:appendix-jump-table.adoc for more information.
====
...
It points to <<amp-5-get-version>> to maintain the numbering of the ``&`` calls.
...
In this respect, it is similar to <<calcgoto>>.
...
This can also be used from BASIC.
...
If specified, __screen_number__ stores the current screen mask number in `curdsp` (`$12e4`), which is used for logic in updating the proper items per screen (_e.g._, the idle timer only when a user is online).
...
[source,basic]
----
375 &,53,a:im=2+a:goto 443 <1>
----

<1> `&,53,a`: Reset flags, store `a` (current screen mask). `im=2+a:goto 443`: display proper screen mask using `sub.display`
...
When `a=0`, this is the "`normal`" entry point into the editor: it starts with an empty buffer.

When `a=1`, entering the editor keeps the existing text in the `tt$()` array, and does not clear the buffer.

When `a=2`, the text editor exits to BASIC in order to check for an extended command.

TIP: Look at `i.WF` for information on how to write an extended command.
There will be some documentation on how to do this, eventually.
...
====
NOTE: These functions are undergoing discovery.
====

Another group of related string conversion functions, there seem to be sub-functions to convert:

* names (?)--this function checks for `'` in a string
* disk data
* special characters to Image-encoded function keys, and
* check for special characters.

====
TIP: Related string conversion functions can be found at <<convan>>.
====

'''

=== `&,66` Calculated `goto` [[calcgoto]]
...
The `a%+1` is necessary because `on-goto` does not recognize when variables equal zero, and would not `goto 4000` otherwise.
====
...
[source,basic]
3000 a%=rnd(1)*14
3010 on a%+1 goto 4000, 4100, 4200, 4300, 4400, 4500, 4600, 4700, 4800, 4900, 5000, 5100, 5200, 5300
4000 rem code
4100 rem continues
4200 rem here
====
...
.&,66: Calculated GOTO
[source,basic]
3000 a%=4000+a%*100:&,66:goto 300
4000 &"&0163;!a{f6}":goto 300:rem print 1
4100 &"&0163;!a{f6}":goto 300:rem print 2
4200 &"&0163;!a{f6}":rem print 3

Based on the value of `a%` (including `0`), `&,66` will `goto` lines starting at `3000` in increments of `100` (`a%=0`: `3000`, `a%=1`: `3100`, `a%=2`: `3200`, `a%=3`: `3300`, etc.)
...
[NOTE]
Within Image BBS`'s line numbering scheme, you are limited to calling lines `1-32767`.
Therefore, calculated ``goto``s cannot be used in a `sub.*` module to call lines from `60000-`.
// TODO: change &,66 to accept a floating point parameter
...
The same thing can be written like so (which uses 21 bytes):

[source]
// TODO: might add this somewhere else, but keep the examples the same
// a%=abs(val(an$)):if a%<15 then a%=3100+a%*50:&,66,1
a%=3000+a%*100:&,66,1

////
fora=0to13:?a,3000+a*100:next
 0         3000
 1         3100
 2         3200
 3         3300
...
[NOTE]
Within Image BBS`'s line numbering scheme, you are limited to calling lines `1-32767`.
Therefore, calculated ``gosub``s cannot be used in a `sub.*` module to call lines from `60000-`.
// TODO: change &,66,1 to accept a floating point parameter
...
=== `&,69` Display String on Console [[display-string-on-console]]

Show a plain text string on the sysop`'s screen.
The position, color, and string are specified in the parameters.
Whether the string is displayed in reverse text or not is a function of the _color_ parameter, shown below.
...
[%autowidth]
|===
| Color Name | Normal | Reversed | Color Name | Normal | Reversed
| white                 |  1 / $01 | 129 / $81
...
| light red             | 10 / $0a | 138 / $8a

| cyan                  |  3 / $03 | 131 / $83

| dark gray             | 11 / $0b | 139 / $8b

| purple                |  4 / $04 | 132 / $84

| gray                  | 12 / $0c | 140 / $8c

| dark green            |  5 / $05 | 133 / $85

| light green           | 13 / $0d | 141 / $8d

| dark blue             |  6 / $06 | 134 / $86

| light blue            | 14 / $0e | 142 / $8e

| yellow                |  7 / $07 | 135 / $87

| light gray            | 15 / $0f | 143 / $8f
