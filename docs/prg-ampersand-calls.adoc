...
* `£m<__x__` (left margin) This MCI command causes every carriage return issued by Image BBS to be followed by _x_ spaces, which indents text _x_ spaces.
The values for _x_ are `0` (to disable word-wrap), or `1-9`, and `j-o` (10-15).

* `£m>__x__` (right margin).
...
does not output `z$` (which outputs `c$`, `£#3` sets 3 leading characters, `£#0` sets the fill character to `0`, and displays the value of `c%`).
...
If it is supplied, needs `,` and the appropriate parameter substituted in its place.
