...
|=====
| Variable | Purpose
| `an$` | a copy of the text the user typed.
Will be a null string (`""`) if the user did not type anything and just hit kbd:[Return] or kbd:[Enter].
|=====
...
<2> a 30-character string (with `+` every 5 characters), used to demonstrate `&,2,2,25`
...
// this part is tricky due to '+...+' being an AsciiDoc passthrough command which displays text literally
#fixme#
// `ml$="++ "+a$`: Assign `ml$` the `++ ` prefix for error reporting purposes.
...
* an entire disk directory at once, from the directory header to the `blocks free.` count
* a line of information at a time

====
[TIP]
(calling it multiple times will get the directory header, each directory entry (containing each file's block count, filename, filetype, splat and lock status), and the `blocks free.` count)
====
...
`&,14,_lfn_`::
Output from 1 to however many elements were ``dim``ensioned for ``tt$()``.

`&,14,_lfn_,_end_`::
Output elements of `tt$()` from 1-_end_.
...
// ----
#FIXME# line number
`xxxx an$="hostkbd:[f3]port":&,15,2:&"£v7kbd:[f6]"`
...
|===
| _mode_ | Purpose
| `0` | Normal entry, empty buffer
| `1` | Buffer not cleared
| `2` | Extended editor command
|===

When `mode=0`, this is the normal entry point into the editor: it starts with an empty buffer.

When `mode=1`, entering the editor keeps the existing text in the `tt$()` array, and does not clear the buffer.

When `mode=2`, the text editor exits to BASIC in order to check for an extended command.
...
The `a%+1` is necessary because `on-goto` does not recognize when variables equal zero, and would not `goto 3000` otherwise.
====

.Calculated GOTO
====
[source,basic]
on a%+1 goto 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800, 3900, 4000, 4100, 4200, 4300
====
...
Whether the string is displayed in reverse text or not is a function of the color parameter, shown below.
