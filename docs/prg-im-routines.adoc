...
This routine exits via `return` on either line `310` <<line-310>> or `314` <<line-314>>.
...
| [[line-9,Display cm$ in Area line of screen mask]]gosub&#160;9
| gosub&#160;1360
| Displays the contents of `cm$` to the `Area` line of the sysop screen mask, unless a network transfer is active (`nt=1`).

This routine exits via either ``goto 999`` <<line-999>> (which ``return``s from there), or a regular `return` at the end of line 9.
...
| [[line-11,Check user flag]]gosub&#160;11
| gosub&#160;1004
...
Each digit represents a separate flag which can be checked by setting the variable `a` to the flag number 1-20 (with the exception of 19 since it is the second byte of the time allowed per day flag), and issuing a `gosub 11` command.
...
| [[line-14,Display a SEQ file]]gosub&#160;14
| gosub&#160;1076
...
1. set `x` = the desired statistic to be updated:

.e.stats records [[e-stats-records]]
...
2. Set `i` to the desired adjustment of the statistic.
This routine continues to line 17.

| [[line-17,Write statistic to e.stats]] gosub&#160;17
| gosub&#160;1026
...
`a$`: should equal `,r` for reading from the file, or `,w` for writing to the file.
...
`x`: the total number of protocols in `tt$()`.
(20 max)
...
| _n/a_
| Display a SEQ file with a `s.` prefix

Sets `dr=1`, the System disk.

_Enter:_

`a$`: the filename, minus the `s.` prefix

_Exit:_

`goto 13` <<line-13>>, which clears the screen and displays the file.

| [[line-43,Display record from e.say]]gosub&#160;43
| gosub&#160;1096
| Display a random saying from the RELative file `e.say`.

_Exit:_

`x`: record number chosen
...
`bf`: Blocks free on the device/drive.
...
[NOTE]
====
`sub.modem` has its own subroutine at line 100.
====

These files replace 2.0`'s `im.*` files.

| `gosub&#160;79`
|
| Loads `i.` module from device/drive `dr`, then ``return``s.

Lines 70-75 fall through to lines 76-78 then to this line before returning and executing at the appropriate line.

| gosub&#160;80
| _n/a_
| Similar to , except uses `p1$` as a reference to the currently loaded file, rather than `pr$`.

| gosub&#160;82
| _n/a_
| Clear arrays `1`-`4` in the <<table-array-numbers>>.

| gosub&#160;83
| _n/a_
| Clear array `0` in the <<table-array-numbers>>.

| [[line-93,Display "Are You Sure? "]]
| gosub 1902
| Display `Are You Sure?: ` and fall through to line 93.

| [[line-94,Wait for Yes/No hotkey, default No]]gosub&#160;96
| gosub&#160;1902
| Display `[No]`.
Wait for yes/no hotkey, the answer defaulting to `No` if anything but kbd:[Y] is typed.

_Exit:_

`a=0`: If kbd:[Y] is typed.
Display `Yes`.
`a=1`: If anything else is typed.
Display `No`.

| [[line-95,Wait for Yes/No hotkey, default Yes]]gosub&#160;96
| gosub&#160;1902
| Wait for yes/no hotkey, the answer defaulting to `Yes`.
...
`a=0`: Anything else kbd:[Y] is typed.
Display `No`.
...
This routine also stores the filename in `cm$`, and does `gosub 9` <<line-9>>.

The drive number and `i.` filename prefixes are automatically added by the subroutine.
...
`is` is the stack depth, and `im$(is)` is the module name.
...
| [[line-300,Main prompt]]300
| _n/a_
| Main prompt routine.
Not to be confused with line 1812 of 1.2's `im` file.
This routine is used for all system prompts.
It prints the prompt in `p$`, the time, and stack free if in local mode.

The routine checks the command stack `uu$`, #FIXME#
...
_Exit:_
On exit, this routine does `goto 3000` when `mm` is set as follows:
...
^*^: for `im=4` or `im-5`, `an$`=filename, and set `x` to one of the following values:
...
| [[line-999,General-purpose `return`]]goto&#160;999
| goto&#160;1603
| General purpose `return` statement.
